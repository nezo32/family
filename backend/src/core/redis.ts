...
/**
 * The exact options every BullMQ connection uses.
 *
 * Exported so tests exercise the real configuration instead of a hand-copied
 * duplicate — the copy in the outage test had already drifted and was asserting
 * against options this file no longer used.
 */
...
export function createBullConnection(): Redis {
  const conn = new Redis(getConfig().REDIS_URL, bullConnectionOptions);
