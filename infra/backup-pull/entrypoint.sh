...
# override survives, which is what makes a one-off run testable by hand.
{
  ...
  for v in TZ SERVER_HOST SERVER_USER REMOTE_DIR GENERATIONS MIN_INTERVAL_HOURS \
           MAX_INTERVAL_HOURS PREFERRED_HOUR CONNECT_TIMEOUT STALE_HOURS PREFIX \
           OBJECTS_MIN_INTERVAL_HOURS OBJECTS_STALE_HOURS ATTIC_KEEP_DAYS \
           ATTIC_ALERT_FILES TICK_STALE_HOURS; do
    eval "val=\${$v:-}"
